...
#!/bin/bash
# CLI Tools Aliases and Shortcuts
# Consolidated aliases for all command line tools
...
alias show-aliases='alias | grep -E "(eza|bat|rg|fd|dust|procs|sd|tldr|glances|hyperfine|git|docker|postgres|mongo)" | sort'
EOF

    print_status "CLI aliases file created at $aliases_file"
    echo "$aliases_file"
}
...
EOF
        ...
        csh|tcsh)
            shell_aliases_file="$HOME/.aliases.csh"
...
setup_modern_cli_aliases() {
    ...
    print_info "Setting up CLI tool aliases..."
    ...
    print_status "CLI aliases setup complete!"
...
#!/bin/bash
# CLI Tools Completions
# Consolidated completions for all command line tools
# Generated by Developer Environment Setup
...
generate_completion "fd" "fd --completion bash" "$BASH_COMPLETION_USER_DIR/fd"
generate_completion "rg" "rg --completion bash" "$BASH_COMPLETION_USER_DIR/rg"
generate_completion "eza" "eza --completion bash" "$BASH_COMPLETION_USER_DIR/eza"
...
if command -v aws &> /dev/null; then
    if ! [ -f "$BASH_COMPLETION_USER_DIR/aws" ]; then
        aws --completion bash > "$BASH_COMPLETION_USER_DIR/aws" 2>/dev/null || true
    fi
fi

# AWS SAM CLI completion
if command -v sam &> /dev/null; then
    if ! [ -f "$BASH_COMPLETION_USER_DIR/sam" ]; then
        sam --completion bash > "$BASH_COMPLETION_USER_DIR/sam" 2>/dev/null || true
    fi
fi
...
# Podman completion
if command -v podman &> /dev/null; then
    if ! [ -f "$BASH_COMPLETION_USER_DIR/podman" ]; then
        podman completion bash > "$BASH_COMPLETION_USER_DIR/podman" 2>/dev/null || true
    fi
fi
...
wait_for_github_setup() {
    ...
    if [ -f "$pub_key_path" ]; then
        print_info "📋 Your SSH public key (copy the line below):"
        echo ""
        echo "----------------------------------------"
        cat "$pub_key_path"
        echo "----------------------------------------"
...
configure_database_environment() {
    ...
    print_status "Database aliases configured in ~/.aliases"
...
add_container_shell_aliases() {
    print_info "Docker compatibility aliases configured in ~/.aliases"
}
