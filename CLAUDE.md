...
#### Key Components:
- **CLI Tool**: `/Users/james/.claude/devenv-setup/bin/omamacy` - Main interface for theme/font management
...
- **User Config**: `~/.config/omamacy/` - User-specific configurations

#### Delegation Pattern:
Individual app scripts respond to environment variables:
- `OMAMACY_APPLY_THEME_ONLY="theme-name"` - Apply only theme settings
- `OMAMACY_APPLY_FONT_ONLY="font-name"` - Apply only font settings
...
#### Commands:
- `omamacy font list` - Opens macOS Font Book for browsing
- `omamacy font set "Font Name"` - Applies font to supported applications
...
- `omamacy theme set <theme>` - Switch to specified theme

#### Font Management:
- `omamacy font list` - Open Font Book for browsing
- `omamacy font set "<font>"` - Set font for supported applications

#### Examples:
```bash
omamacy theme set catppuccin-latte
omamacy font set "JetBrains Mono Nerd Font"
omamacy font set "SF Mono"
```
...
3. **Centralized Control**: `omamacy` CLI maintains control over theme/font switching
...
#### Enhanced Scripts:
All app scripts now support `OMAMACY_APPLY_THEME_ONLY` and `OMAMACY_APPLY_FONT_ONLY` environment variables for delegation-based updates.
